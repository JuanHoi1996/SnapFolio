...
仓库根目录的 `packages.txt` 会在云端安装 `libgl1`、`libglib2.0-0t64`（Streamlit Cloud 使用 Debian Trixie，glib 包名带 `t64` 后缀）；`requirements.txt` 优先使用 `opencv-python-headless`。推送后请在 Streamlit Cloud 控制台 **Reboot app**（不是 Rerun），等依赖重新构建完成再试识别。
